...
namespace KilowaySDK.ActiveMQ
{
    ...
    /// </summary>
    public class ActiveMQConsumer:AbstractActiveMQ,IConsumer,IMessageQueue,IDisposable
    {
        /// <summary>
        ...
        /// </summary>
        public Action<BaseMessage> OnBaseMessageReceived { get; set; }
        ...
        public void StartListen()
        {
            ...
            _consumer.Listener += new MessageListener(msg =>
            {
                ...
                //转换为业务需要的数据对象
                if (OnBaseMessageReceived != null)
                {
                    var objectMessage = msg as ActiveMQObjectMessage;
                    if (objectMessage != null)
                    {
                        var dataCenterMsg = objectMessage.Body as BaseMessage;
                        if (dataCenterMsg != null)
                        {
                            OnBaseMessageReceived(dataCenterMsg);
                        }
                    }
                }
            });
